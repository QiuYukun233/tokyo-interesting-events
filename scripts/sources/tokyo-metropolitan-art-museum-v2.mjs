...
function isCurrentOrUpcoming(section) {
  const heading = compact(section.find('.section-header-title').first().text());
  return /開催中|これから開催/.test(heading);
}

/** Parse current and upcoming exhibition cards from the museum's official listing. */
export function parseTokyoMetropolitanArtMuseum(html, source) {
  ...
  $('section').each((_, sectionNode) => {
    ...
    section.find('a.exhibition-item').each((index, node) => {
      const item = $(node);
      const title = compact(item.find('.-title').text());
      ...
      const candidate = createEventCandidate({
        ...
        price: '详见活动页',
        text: category,
        visualIndex: events.length + index,
      });
