...
const NOT_A_SHOP_CHANGE = /周年記念|記念企画|フェア|セール|販売|イベント|キャンペーン/;
...
function changeTypeFor(text) {
  ...
  if (DISCOVERY.test(text) && !NOT_A_SHOP_CHANGE.test(text)) return 'discovery';
  return null;
}
...
function describedChangeDate(description, publishedDate) {
  const m = description.match(/(\d{1,2})月\s*(\d{1,2})日/); if (!m || !publishedDate) return null;
  const published = new Date(`${publishedDate}T00:00:00+09:00`); let year = published.getFullYear(); const month = Number(m[1]);
...
/** Lead extraction from robots-permitted HTML only; the RSS endpoint is disallowed. */
export function parseIkebukuroKeizaiHomepage(html, source) {
  ...
  $('a[href*="/headline/"]').each((_, node) => {
    const link = $(node); const href = link.attr('href'); if (!href || seen.has(href)) return;
    const title = compact(link.find('h2, h3, h4').first().text()); const changeType = changeTypeFor(title);
    if (!title || !changeType) return;
    seen.add(href); leads.push({ title, changeType, category: compact(link.find('span').first().text()), sourceUrl: new URL(href, source.url || IKEBUKURO_KEIZAI_HOME_URL).href });
  });
  return leads;
}

/** Article metadata is transient: only dates/place/type/attribution leave the adapter. */
export function parseIkebukuroKeizaiArticle(html, lead, source) {
  const $ = cheerio.load(html); const metadata = compact($('meta[name="description"]').attr('content') || ''); const publishedDate = isoDate(compact($('time').first().text()));
...
export async function collectIkebukuroKeizaiShopChanges({ source, fetchImpl = fetch, maxArticles = 12, now = new Date(), maxAgeDays = 45 }) {
  const homepage = await fetchImpl(source.url || IKEBUKURO_KEIZAI_HOME_URL); if (!homepage.ok) throw new Error(`${source.name} homepage returned ${homepage.status}`);
  const leads = parseIkebukuroKeizaiHomepage(await homepage.text(), source).slice(0, maxArticles); const events = []; const oldest = new Date(now.getTime() - maxAgeDays * 86400000);
