...
import { SOURCES } from './sources/index-v2.mjs';
...
const USER_AGENT = 'TokyoInterestingEvents/0.3 (+contact via repository)';
...
function mergeEvents({ manual, fetched, existing, now, limit = 50, horizonDays = 180 }) {
...
async function main() {
  ...
  if (!successful.length) throw new AggregateError(failed.map((result) => result.reason), 'All event sources failed');
  for (const result of failed) console.error(`Source failed: ${result.reason?.message || result.reason}`);
  ...
  await Promise.all([
    writeFile(OUTPUT, `${JSON.stringify({ updatedAt: tokyoNow, updatedAtLabel, events }, null, 2)}\n`),
    writeFile(REVIEW_OUTPUT, `${JSON.stringify({ updatedAt: tokyoNow, events: review }, null, 2)}\n`),
  ]);
