...
const cand = (overrides = {}) => ({
  ...
  ongoing: false, tags: [], objectType: 'place', ...overrides,
});
...
test('a broad default pool shares 15 items across at most five families', () => {
  const pool = Array.from({ length: 15 }, (_, i) => cand({
    id: `family-${i}`, sourceFamily: `family-${i % 5}`, score: 100 - i,
  }));
  const round = buildRound(pool, { now: NOW, size: 15 });
  assert.equal(round.length, 15);
  const counts = new Map();
  for (const item of round) counts.set(item.sourceFamily, (counts.get(item.sourceFamily) ?? 0) + 1);
  assert.ok(Math.max(...counts.values()) <= 3, JSON.stringify(Object.fromEntries(counts)));
});

test('an explicit familyCap overrides the adaptive default', () => {
  const pool = Array.from({ length: 15 }, (_, i) => cand({
    id: `family-${i}`, sourceFamily: `family-${i % 5}`, score: 100 - i,
  }));
  const round = buildRound(pool, { now: NOW, size: 15, familyCap: 1 });
  assert.equal(round.length, 5);
});

test('comprehensive rounds cap events and exhibitions but fill with places and activities', () => {
  const pool = [
    ...Array.from({ length: 10 }, (_, i) => cand({
      id: `event-${i}`, objectType: 'event', sourceFamily: `event-${i}`, score: 100 - i,
    })),
    ...Array.from({ length: 8 }, (_, i) => cand({
      id: `exhibition-${i}`, objectType: 'exhibition', sourceFamily: `exhibition-${i}`,
      startDate: '2026-09-10', endDate: '2026-09-20', score: 50 - i,
    })),
    ...Array.from({ length: 4 }, (_, i) => cand({
      id: `place-${i}`, objectType: 'place', sourceFamily: `place-${i}`, score: 1,
    })),
    ...Array.from({ length: 4 }, (_, i) => cand({
      id: `activity-${i}`, objectType: 'activity', sourceFamily: `activity-${i}`, score: 1,
    })),
  ];
  const round = buildRound(pool, { now: NOW, size: 15 });
  const counts = round.reduce((all, item) => ({ ...all, [item.objectType]: (all[item.objectType] ?? 0) + 1 }), {});
  assert.equal(round.length, 15);
  assert.ok((counts.event ?? 0) <= 5, JSON.stringify(counts));
  assert.ok((counts.exhibition ?? 0) <= 3, JSON.stringify(counts));
  assert.ok(counts.place > 0, JSON.stringify(counts));
  assert.ok(counts.activity > 0, JSON.stringify(counts));
});

test('tag-focused rounds do not shorten when all picks share one type', () => {
  const pool = Array.from({ length: 15 }, (_, i) => cand({
    id: `exhibition-${i}`, objectType: 'exhibition', sourceFamily: `family-${i}`,
    startDate: '2026-09-10', endDate: '2026-09-20', tags: ['art'], score: 10 - i,
  }));
  const round = buildRound(pool, { now: NOW, size: 15, tag: 'art' });
  assert.equal(round.length, 15);
  assert.equal(new Set(round.map((item) => item.objectType)).size, 1);
});
