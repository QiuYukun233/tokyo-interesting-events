 ...
 *   popularity     ≤0.5      source-reported interest count; capped at the
 *                            weight of one evidence code because ~85% of the
 *                            pool sits at 0 — it separates "a dozen people
 *                            care" from "no data", nothing finer
...
export function scoreFor(candidate = {}, weights = new Map(), { now } = {}) {
  ...
  if (candidate.popularity > 0) score += Math.min(candidate.popularity, 20) / 40;
...
export function rankCandidates(candidates = [], { weights = new Map(), now } = {}) {
  const scored = candidates.map((candidate) => ({
    ...candidate,
    // `displayTags`, not `tags`: the candidate may already carry the explore
    // queue's vocabulary tags (pool-db's tags table), and spreading these card
    // labels over that field would silently break lib/queue.mjs's tag scoring.
    displayTags: tagsFor(candidate),
