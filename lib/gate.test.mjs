...
test('a funeral-goods retailer is ruled out as not a destination', () => {
  const decision = gateDecision(candidate({ reasons: ['hard:funeral_supplies'] }));
  assert.equal(decision.decidedBy, 'rule:not_a_destination');
  assert.equal(decision.state, 'rejected');
});

test('career fairs and admission sessions are auto-rejected; broad recruiting evidence alone is not', () => {
  // 2026-09-01: the queue put pure job/admission events in front of a human
  // and every one came back 不想去; the 4 previously human-decided
  // hard:recruiting candidates were all rejected too. hard:career_event and
  // hard:admissions are verdict-grade (see activity-filter); hard:recruiting
  // stays evidence-only because 「採用した」 trips it on a leather workshop.
  const base = { state: 'pending', title: 'x' };
  const career = gateDecision({ ...base, reasons: ['hard:career_event', 'hard:recruiting'] });
  assert.equal(career?.state, 'rejected');
  assert.equal(career?.decidedBy, 'rule:not_a_destination');
  const admission = gateDecision({ ...base, reasons: ['hard:admissions'] });
  assert.equal(admission?.state, 'rejected');
  assert.equal(gateDecision({ ...base, reasons: ['hard:recruiting'] }), null);
});
