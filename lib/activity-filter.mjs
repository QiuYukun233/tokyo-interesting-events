...
const HARD_EXCLUSION_RULES = [
  // Verdict-grade career/admission events: patterns that cannot mean anything
  // but a job fair, company briefing, or school-admission session. The broad
  // hard:recruiting below stays as evidence only — 「採用した」 (a mechanism
  // was adopted) trips it on a leather workshop. This code the gate trusts
  // outright: every pure career/admission event surfaced in the queue on
  // 2026-09-01 was voted 不想去, and all 4 human-decided hard:recruiting
  // candidates before that were rejected.
  { code: 'hard:career_event', label: '纯就活・招聘活动', pattern: /就職面接会|合同企業説明会|会社説明会|合同(?:就職)?説明会|就職フォーラム|就活|転職フェア|転職イベント|採用イベント|求人フェア|職業紹介|インターンシップ|キャリア(?:フェア|セミナー|塾|支援|アップ)/i },
  { code: 'hard:career_event', label: '纯就活・招聘活动', pattern: /招聘会|校招|宣讲会|求职/i },
