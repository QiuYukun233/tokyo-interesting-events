...
export function splitForCloud(candidates = [], { now } = {}) {
  ...
  for (const candidate of candidates) {
    if (candidate.state === 'rejected' || !candidate.taggedBy) {
      // The default queue must never consume an untagged candidate. Include it
      // in deletes so a stale mirror row is removed on the next push.
