...
/** The user's subscribed tags — the queue's primary source when non-empty. */
export async function listSubscriptions(client) {
  const result = await client.execute('SELECT tag FROM subscriptions');
  return result.rows.map((row) => row.tag);
}

/** Replace-all semantics: the UI always sends the full set, so a partial
 * write cannot strand a stale subscription. */
export async function setSubscriptions(client, tags = [], { now } = {}) {
  const addedAt = new Date(now ?? Date.now()).toISOString();
  await client.batch([
    { sql: 'DELETE FROM subscriptions', args: [] },
    ...[...new Set(tags)].map((tag) => ({
      sql: 'INSERT INTO subscriptions (tag, addedAt) VALUES (?, ?)', args: [tag, addedAt],
    })),
  ], 'write');
}
