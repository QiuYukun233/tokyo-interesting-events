 ...
 *
 * Comprehensive rounds cap event/exhibition counts so places and
 * activities can fill the queue. Tag-focused and subscribed rounds lift those
 * type caps so a real topic is not shortened just because it has one type.
 * The family cap adapts to available breadth: at most five families share the
 * round, while an explicit familyCap still takes precedence.
 *
...
export function buildRound(candidates = [], {
  ...
  const typeOf = (c) => c.objectType ?? objectTypeFor(c);
  const subs = new Set(subscribedTags);
  const familyCount = new Set(eligible.map(familyOf)).size;
  // Share the round across at most five available families. For a 15-item
  // round this yields 3 each with 5+ families, 5 each with 3 families.
  const cap = familyCap ?? Math.ceil(size / Math.min(5, Math.max(1, familyCount)));
  const typeCaps = !tag && !subs.size ? { event: 5, exhibition: 3 } : null;
  const familyPicked = new Map();
  const typePicked = new Map();
  const picked = [];
  const taken = new Set();
  const take = (candidate, reason) => {
    ...
    if ((familyPicked.get(family) ?? 0) >= cap) return false;
    const type = typeOf(candidate);
    if (typeCaps?.[type] !== undefined && (typePicked.get(type) ?? 0) >= typeCaps[type]) return false;
    familyPicked.set(family, (familyPicked.get(family) ?? 0) + 1);
    typePicked.set(type, (typePicked.get(type) ?? 0) + 1);
