...
function candidate(overrides = {}) {
  return {
    ...
    decidedBy: null, taggedBy: 'ai:test', ...overrides,
  };
}
...
test('rejected and untagged candidates are deleted, while tagged empty rows are pushed', () => {
  const rows = [
    candidate({ id: 'tagged-empty', tags: [], taggedBy: 'ai:test' }),
    candidate({ id: 'untagged', taggedBy: null }),
    candidate({ id: 'rejected', state: 'rejected', taggedBy: 'ai:test' }),
  ];
  const { pushRows, deleteIds } = splitForCloud(rows, { now: NOW });
  assert.deepEqual(pushRows.map((r) => r.id), ['tagged-empty']);
  assert.deepEqual(deleteIds.sort(), ['rejected', 'untagged']);
});
