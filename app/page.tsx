        ...
        <div className="event-grid">
          {visible.map((event, index) => {
            ...
            return <article className="event-card" key={event.id}>
              ...
              <div className="event-body"><span className="vibe">{event.vibe}</span><h3 lang="ja">{event.title}</h3><p className="event-zh">{event.titleZh}</p><dl><div><dt>地点</dt><dd>{event.place}</dd></div><div><dt>时间</dt><dd>{event.time}</dd></div><div><dt>费用</dt><dd>{event.price}</dd></div></dl><a className="source-link" href={event.sourceUrl} target="_blank" rel="noreferrer">查看主办方页面 ↗</a><div className="mate-row"><div className="faces"><span>ア</span><span>林</span><span>J</span></div><p><b>{event.mates + index} 人</b>想找人一起</p><button onClick={() => setJoining(event)}>{joined.includes(event.id) ? '已发起 ✓' : '我也想去 ↗'}</button></div></div>
            </article>;
          })}
        </div>
      </section>
