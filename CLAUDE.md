...
3. **候选量差约 16 倍。** 当前约每周 60 条，方案 §10 的漏斗要 1,000 条；
   前台 34 条里只有 9 条在未来两周内开始，探索队列（12–20 项/轮）勉强够一个人一轮。
   类别也偏：方案 §3.2 的六个来源家族里「新运动与参与式消遣」「小型文化与亚文化」
   两个是 0。**最大的杠杆不是加新来源，是已有来源严重欠采**——Big Sight 用 HTML
   爬到 10 条，官方开放数据有 154 条；みんなの経済新聞我们接了 2 个版，
   东京圈有 15 个版且共用同一套解析。方案见 `docs/信息获取管道设计.md`。
